...
- POST /upload/receipt-journal: レシートジャーナルCSVのアップロード
"""
...
@router.post(
    "/receipt-journal",
    summary="レシートジャーナルCSVをアップロード",
    description="""
    POSレシートジャーナルCSVファイルをアップロードして処理する。

    ## CSVフォーマット
    - エンコーディング: Shift-JIS (cp932) / UTF-8
    - 1行目: 期間情報（例: 販売日：2026年3月1日(日) 〜 2026年3月3日(火)）
    - 2行目以降: ヘッダー + データ行
    - カラム: 販売日時, レシート番号, 商品名, 数量, 小計 等

    ## 処理内容
    1. エンコーディング自動判定
    2. 期間・販売日時・店舗コード・商品名のパース
    3. 商品グループの自動判定
    4. (date, hour, segment_id, product_name) で集計
    5. hourly_salesテーブルへのUpsert

    ## 店舗コード
    レシート番号 "No.25-251-..." の先頭数値から抽出
    """,
    responses={
        200: {
            "description": "インポート成功",
...
async def upload_receipt_journal(
    file: UploadFile = File(..., description="レシートジャーナルCSV"),
    current_user: User = Depends(get_current_user),
    supabase: Client = Depends(get_supabase_admin),
):
    """レシートジャーナルCSVをアップロードして処理する"""
    from app.services.receipt_journal_parser import parse_receipt_journal
    from app.services.daily_sales_import_service import import_receipt_journal
    from app.schemas.daily_sales import ReceiptJournalUploadResult
    ...
    # パース
    parsed = parse_receipt_journal(content, file.filename or "")
    ...
    if not parsed["success"]:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="レシートジャーナルCSVのパースに失敗しました"
        )

    # インポート
    try:
        import_result = await import_receipt_journal(supabase, parsed)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"データのインポートに失敗しました: {str(e)}"
    ...
    return ReceiptJournalUploadResult(
        success=len(all_errors) == 0,
