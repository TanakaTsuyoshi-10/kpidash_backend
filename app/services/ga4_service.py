...
# GA4プロパティ（日本）のタイムゾーン。「昨日」判定をJST基準で行う
# （Cloud RunはUTC稼働のため date.today() では早朝に日付がずれる）
...
def _run_report(client: Any, property_id: str) -> Dict[str, Any]:
    ...
    # JST基準で「昨日」を判定（Cloud RunのUTC稼働による日付ずれを防ぐ）
...
@cached(prefix="ga4", ttl=1800)
async def get_ec_summary() -> Dict[str, Any]:
    """EC Web分析サマリーを取得する（TTL 1800秒＝30分）。
